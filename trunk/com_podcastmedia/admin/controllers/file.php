	...
	function upload()
	{
		...
		if (isset($file['name']))
		{
			...
			else
			{
				...
				$this->setMessage(JText::sprintf('COM_MEDIA_UPLOAD_COMPLETE', substr($file['filepath'], strlen(COM_PODCASTMEDIA_BASE))));
				return true;
			}
		}
		else
			...
			if (count($paths))
			{
				...
				foreach ($paths as $path)
				{
					if ($path !== JFile::makeSafe($path))
					{
						...
						JError::raiseWarning(100, JText::sprintf('COM_MEDIA_ERROR_UNABLE_TO_DELETE_FILE_WARNFILENAME', substr($filename, strlen(COM_PODCASTMEDIA_BASE))));
						continue;
					}
					...
					if (is_file($fullPath))
					{
						...
						$this->setMessage(JText::sprintf('COM_MEDIA_DELETE_COMPLETE', substr($fullPath, strlen(COM_PODCASTMEDIA_BASE))));
					}
					else if (is_dir($fullPath))
					{
						...
						{
							//This makes no sense...
							JError::raiseWarning(100, JText::sprintf('COM_MEDIA_ERROR_UNABLE_TO_DELETE_FOLDER_NOT_EMPTY',substr($fullPath, strlen(COM_PODCASTMEDIA_BASE))));
						}
					}
				}
			}
